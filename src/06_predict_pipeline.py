...
#day_b4_yesterday_date = (pd.to_datetime( today_date )-timedelta(days=2)).strftime("%Y-%m-%d")
#two_days_b4_yesterday_date = (pd.to_datetime( today_date )-timedelta(days=3)).strftime("%Y-%m-%d")
dates = pd.date_range( pd.to_datetime(week_ago_date), pd.to_datetime( today_date ) + timedelta(1) , freq='d')
...
print(' ----------------------------------------------------------------------------------------')
print('************************************ Feature build and building test set *********************************')
print(' ----------------------------------------------------------------------------------------')
...
for file in file_names:
 ...
 #       df_two_days_b4_yesterday =   df[df.index.date == pd.to_datetime(two_days_b4_yesterday_date).date()]
 #       df_day_b4_yesterday =   df[df.index.date == pd.to_datetime(day_b4_yesterday_date).date()]
 #       df_yesterday=   df[df.index.date == pd.to_datetime(yesterday_date).date()]


 #       test_set_two_days_b4_yesterday = df_two_days_b4_yesterday.values.reshape(24*8)
 #       test_set_day_b4_yesterday = df_day_b4_yesterday.values.reshape(24*8)
 #       test_set_yesterday = df_yesterday.values.reshape(24*8)
 ...
 #       path_to_test_data_file = os.path.join( dir_test_data,  file.split('/')[2].split('.')[0]  )
 #       print(path_to_test_data_file)
 #       np.savez_compressed(path_to_test_data_file + two_days_b4_yesterday_date + '.npz', test_set_two_days_b4_yesterday)
 #       np.savez_compressed(path_to_test_data_file + day_b4_yesterday_date + '.npz', test_set_day_b4_yesterday)
 #       np.savez_compressed(path_to_test_data_file + yesterday_date + '.npz', test_set_yesterday)




print(' ----------------------------------------------------------------------------------------')
print('************************************ Live Prediction from the latest available data *********************************')
print(' ----------------------------------------------------------------------------------------')
