...
with st.sidebar:
    st.warning('*Hover over the circles to see the junction name, traffic and change in % from prior hr*', icon="📌")
...
with col4:
        st.markdown('**Hourly Traffic prediction for 8 junctions since the past 7 days and upto 3 hours in future:**')
        for file in file_names:
                print(file)
                df = pd.read_csv(file,index_col=0)
